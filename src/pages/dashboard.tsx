...
import { getZoomMeetings } from 'apiSdk/zoom-meetings';
import { ZoomMeetingInterface } from 'interfaces/zoom-meeting';
import { getAttendances } from 'apiSdk/attendances';
import { AttendanceInterface } from 'interfaces/attendance';

import { ZoomMeetingListPage as ShowCaseList } from 'pages/zoom-meetings';
...
function DashboardPage() {
  ...
  const zoomMeetingStat = useSWR<PaginatedInterface<ZoomMeetingInterface>>(
    () => `/zoom-meetings`,
    () => getZoomMeetings({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Zoom Meetings',
      result: zoomMeetingStat?.data?.totalCount,
      isLoading: zoomMeetingStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Attendances',
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Zoom Meetings
                </Text>
                <NextLink href={`zoom-meetings`} passHref legacyBehavior>
                  <Button
