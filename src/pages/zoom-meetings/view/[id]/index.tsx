...
import { getZoomMeetingById } from 'apiSdk/zoom-meetings';
import { ZoomMeetingInterface } from 'interfaces/zoom-meeting';

function ZoomMeetingViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<ZoomMeetingInterface>(
    () => (id ? `/zoom-meetings/${id}` : null),
    () =>
      getZoomMeetingById(id, {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('zoom_meeting', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/zoom-meetings/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Meeting Link" text={data?.meeting_link} />

                <FormListItem
                  label="Meeting Date"
                  text={
                    data?.meeting_date ? format(parseISO(data?.meeting_date as unknown as string), 'dd-MM-yyyy') : ''
                  }
                />
